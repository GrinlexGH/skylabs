...
namespace Log {
PUBLIC_CLASS void AddSink(std::unique_ptr<ISink> sink) { g_sinks.push_back(std::move(sink)); }

void CConsoleSink::Write(const Category category, const Level level, const std::source_location& /*loc*/,
...
PUBLIC_CLASS void SubmitLog(const Category category, const Level level, const std::source_location& loc,
                            const std::string& message) {
