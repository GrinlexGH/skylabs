...
PUBLIC_CLASS void SubmitLog(Category category, Level level, const std::source_location& loc,
...
struct Format {
    ...
    consteval Format(const T& format, const std::source_location& sourceLocation =
        ...
        : m_format(format), m_sourceLocation(sourceLocation) {
    }
};
...
void DoLog(const Category category, const Level level, Format<std::type_identity_t<Args>...> fmt,
           Args&&... args) {
    SubmitLog(category, level, fmt.m_sourceLocation,
              fmt::format(fmt.m_format, std::forward<Args>(args)...));
}
