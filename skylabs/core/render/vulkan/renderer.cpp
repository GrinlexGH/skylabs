...
void Renderer::Draw(const glm::mat4 view, const float fov, float /*deltatime*/) {
    ...
    std::ignore =
        device->waitForFences({ m_fence.Get() }, vk::True, std::numeric_limits<std::uint64_t>::max());
    ...
    if (m_firstUse.Get()) {
        cmd.PipelineBarrier({
            ImageBarrier { mainColor, mainColor.FullRange(), Usage::eNone, Usage::eColorAttachment },
            ImageBarrier { mainColorMSAA, mainColorMSAA.FullRange(), Usage::eNone,
                           Usage::eColorAttachment },
            ImageBarrier { mainDepthMSAA, mainDepthMSAA.FullRange(), Usage::eNone, Usage::eDepthWrite },
        });
        m_firstUse.Get() = false;
    } else {
        cmd.PipelineBarrier({
            ImageBarrier { mainColor, mainColor.FullRange(), Usage::eSampledFragment,
                           Usage::eColorAttachment },
        });
    }
    ...
    cmd.PipelineBarrier(
        { ImageBarrier { mainColor, mainColor.FullRange(), Usage::eColorAttachment,
                         Usage::eSampledFragment },
...
//                 cmd.PipelineBarrier({ ImageBarrier { texture, texture.FullRange(),
//                 Vulkan::Usage::eNone, Vulkan::Usage::eTransferWrite } }); cmd.Copy(texture,
//                 m_stagingBuffer); cmd.GenerateMipmaps(texture);
//             }
//         );
...
//         allocationInfo.setSize(vSize);
//         mesh.vtxAlloc = vma::raii::VirtualAllocation { m_vertexBuffer.VirtualBlock(), allocationInfo
//         }; allocationInfo.setSize(iSize); mesh.idxAlloc = vma::raii::VirtualAllocation {
//         m_indexBuffer.VirtualBlock(), allocationInfo }; mesh.indexCount =
...
//         std::memcpy(m_stagingBuffer.Data(), vertices.data(), vSize);
//         std::memcpy(static_cast<std::uint8_t*>(m_stagingBuffer.Data()) + vSize, indices.data(),
//         iSize);
...
//     m_objects.emplace_back(0, 0, glm::rotate(glm::translate(glm::identity<glm::mat4>(), glm::vec3 { 0,
//     0, 1 }), glm::radians(90.0f), glm::vec3 { 1, 0, 0 })); m_objects.emplace_back(1, 1);
// }
}
