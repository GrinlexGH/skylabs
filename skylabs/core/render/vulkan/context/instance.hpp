...
public:
    ...
    [[nodiscard]] auto operator->() const noexcept -> const vk::raii::Instance* { return &m_handle; }
    [[nodiscard]] auto Handle() const noexcept -> const vk::raii::Instance& { return m_handle; }
    ...
    [[nodiscard]] auto PhysicalDevices() noexcept -> std::vector<CPhysicalDevice>& { return m_physicalDevices; }

private:
    auto EnableLayers(std::span<std::string_view> requestedLayers) -> std::vector<const char*>;
    auto EnableExtensions(std::span<Utils::CRequestedExtension> requestedExtensions) -> std::vector<const char*>;
