...
input::Event EventPump::TranslateEvent(const SDL_Event& event) {
    switch (event.type) {
        case SDL_EVENT_QUIT:
            return input::QuitEvent { };
        case SDL_EVENT_RENDER_DEVICE_RESET:
            return input::DeviceResetEvent { };
        case SDL_EVENT_WINDOW_PIXEL_SIZE_CHANGED:
            return input::WindowResizeEvent { .width = event.window.data1,
                                              .height = event.window.data2 };
        case SDL_EVENT_WINDOW_EXPOSED:
            return input::WindowExposeEvent { };
        case SDL_EVENT_KEY_DOWN:
        case SDL_EVENT_KEY_UP:
            if (!kKeyMap.contains(event.key.key)) return input::UnknownEvent { };
            return input::KeyEvent { .key = kKeyMap.at(event.key.key),
                                     .down = event.type == SDL_EVENT_KEY_DOWN };
        case SDL_EVENT_MOUSE_MOTION:
            return input::MouseMotionEvent { .dx = event.motion.xrel, .dy = event.motion.yrel };
        case SDL_EVENT_MOUSE_WHEEL:
            return input::MouseWheelEvent { event.wheel.y };
        case SDL_EVENT_MOUSE_BUTTON_DOWN:
        case SDL_EVENT_MOUSE_BUTTON_UP:
            return input::MouseButtonEvent { event.type == SDL_EVENT_MOUSE_BUTTON_DOWN };
        case SDL_EVENT_FINGER_DOWN:
        case SDL_EVENT_FINGER_UP:
            return input::FingerTouchEvent { .down = event.type == SDL_EVENT_FINGER_DOWN,
                                             .x = event.tfinger.x,
                                             .y = event.tfinger.y,
                                             .fingerID =
                                                 static_cast<unsigned int>(event.tfinger.fingerID) };
        case SDL_EVENT_FINGER_MOTION:
            return input::FingerMotionEvent { .x = event.tfinger.x,
                                              .y = event.tfinger.y,
                                              .dx = event.tfinger.dx,
                                              .dy = event.tfinger.dy,
                                              .fingerID =
                                                  static_cast<unsigned int>(event.tfinger.fingerID) };
        default:
            return input::UnknownEvent { };
    }
}

bool EventPump::EventFilterWrap(void* userData, SDL_Event* const event) {
    const auto eventPump = static_cast<EventPump*>(userData);
