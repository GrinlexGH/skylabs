...
template <typename T> using size_result = decltype(std::declval<T>().size());
...
/// `size()` returning an integral, and a member function `data()` returning a C
/// string
...
///
/// If there is not enough room in the buffer NULL is returned, and the content
